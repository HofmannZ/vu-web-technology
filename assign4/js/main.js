...
// Sorts the product inventory
function sortInventory(productInventory, field, callback) {
  var sorted = false;
  while (!sorted) {
    sorted = true;
    for (var i = 0; i < productInventory.length - 1; i++) {
      if (productInventory[i][field] > productInventory[i + 1][field]) {
        [productInventory[i], productInventory[i + 1 ]] =
          swap(productInventory[i], productInventory[i + 1]);
        sorted = false;
      }
    };
  };
  callback(productInventory);
};
...
function renderSortedInventory(field) {
  fetchInventory(function(productInventory) {
    sortInventory(productInventory, field, function(productInventorySorted) {
      renderInventory(productInventorySorted);
    });
  });
};
...
window.onload = function() {
  ...
  // Check if the head is clicked and sort.
  name.addEventListener('click', function() {
    renderSortedInventory('name');
  });
  category.addEventListener('click', function() {
    renderSortedInventory('category');
  });
  amount.addEventListener('click', function() {
    renderSortedInventory('amount');
  });
  location.addEventListener('click', function() {
    renderSortedInventory('location');
  });
  date.addEventListener('click', function() {
    renderSortedInventory('date');
  });
